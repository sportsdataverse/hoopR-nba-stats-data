...
def detect_missing_seasons(published: set[int], current: int, first: int = 2021) -> list[int]:
    ...
    Args:
        published: Season end-years already on the release (parsed from asset names).
        current: The target through-season end-year (inclusive).
        first: The earliest season to consider (default 2021, the harness report floor).

    Returns:
        Ascending list of missing season end-years (empty if all present).
    """
