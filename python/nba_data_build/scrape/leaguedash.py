"""Season-level league-dash scrape for stats.nba.com — the FULL parameter
cube of the ``leaguedash*`` endpoints.

NBA-only: the WNBA half of this module moved to ``wehoop-wnba-stats-data``'s
``python/wnba_data_build/`` (WNBA has its own producer repo, mirroring the
``hoopR``/``wehoop`` R-package split; this repo's name is NBA-specific).
...
exactly 2000). Per-team fetches (``team_id_nullable``, 30 calls/slice) are the
workaround if full lineup coverage is ever needed.
...
_MODULE = "sportsdataverse.nba.nba_stats"
_SLUG_PREFIX = "nba_stats"
_LEAGUE_ID = "00"
...
class Variant:
    ...
    Attributes:
        ...
        slug: the ``nba_stats_`` wrapper suffix.
...
def megas() -> tuple[str, ...]:
    """Mega tables assemblable from the NBA cube, in build order."""
    return tuple(dict.fromkeys(v.mega for v in variants() if v.mega))


def season_str(year: int) -> str:
    """stats.nba.com season string: ``2024`` -> ``"2023-24"``."""
