...
from nba_data_build.publish import upload_artifacts, plan_uploads, published_seasons


def _mk(tmp_path):
    d = tmp_path / "rapm"; d.mkdir()
...
def test_upload_creates_release_when_missing_then_uploads(tmp_path):
    d = _mk(tmp_path)
    calls = []
    res = upload_artifacts(d, tag="nba_stats_rapm", repo="sportsdataverse/sportsdataverse-data",
                           runner=lambda args: calls.append(args), exists_check=lambda t, r: False)
...
def test_upload_skips_create_when_release_exists(tmp_path):
    d = _mk(tmp_path)
    calls = []
    upload_artifacts(d, tag="nba_stats_rapm", repo="r/r",
                     runner=lambda args: calls.append(args), exists_check=lambda t, r: True)
...
def test_dry_run_uploads_nothing(tmp_path):
    d = _mk(tmp_path)
    calls = []
    res = upload_artifacts(d, tag="t", repo="r/r", dry_run=True,
                           runner=lambda args: calls.append(args), exists_check=lambda t, r: True)
