"""Tests for the NBA league-dash cube scrape (offline — injected transport)."""
...
def test_season_str() -> None:
    assert season_str(2024) == "2023-24"
    assert season_str(2000) == "1999-00"  # zero-pad the tail
...
def test_build_megas_assemble_from_disk_state(tmp_path) -> None:
    ...
    tag_dir = tmp_path / "nba_stats_leaguedash"
    ...
    assert written == {"nba_stats_leaguedash/player_master": 1}
